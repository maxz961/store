export const s = {
  page: 'mx-auto max-w-3xl px-4 py-8 sm:px-6',
  title: 'text-2xl font-semibold text-foreground',
  subtitle: 'mt-1 text-sm text-muted-foreground',
  backLink: 'inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-foreground transition-colors duration-150 mb-4',

  // Status section
  statusCard: 'mt-6 rounded-xl border border-border bg-card p-6 shadow-sm',
  statusTitle: 'text-sm font-medium text-muted-foreground uppercase tracking-wider',
  statusButtons: 'mt-3 flex flex-wrap gap-2',
  statusBtn: 'rounded-lg px-3.5 py-1.5 text-sm font-medium transition-colors duration-150',
  statusBtnActive: 'bg-primary text-primary-foreground',
  statusBtnInactive: 'border border-border text-muted-foreground hover:bg-accent hover:text-foreground',

  // Info grid
  infoGrid: 'mt-6 grid grid-cols-1 gap-6 sm:grid-cols-2',
  infoCard: 'rounded-xl border border-border bg-card p-5 shadow-sm',
  infoTitle: 'text-sm font-medium text-muted-foreground uppercase tracking-wider',
  infoValue: 'mt-2 text-sm text-foreground leading-relaxed',

  // Items
  itemsCard: 'mt-6 rounded-xl border border-border bg-card shadow-sm',
  itemsTitle: 'px-5 pt-5 text-sm font-medium text-muted-foreground uppercase tracking-wider',
  itemRow: 'flex items-center justify-between px-5 py-3.5 border-b border-border last:border-0',
  itemName: 'text-sm font-medium text-foreground',
  itemQty: 'text-xs text-muted-foreground mt-0.5',
  itemPrice: 'text-sm font-medium text-foreground',
  totalRow: 'flex items-center justify-between px-5 py-4 bg-muted/30 rounded-b-xl',
  totalLabel: 'text-sm font-semibold text-foreground',
  totalValue: 'text-lg font-semibold text-foreground',

  // Loading skeleton
  skeleton: 'animate-pulse rounded-xl border border-border bg-muted/50',
};
