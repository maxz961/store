...
export const createPromotionFormSchema = z.object({
  ...
  description: z.string().max(500, 'Max 500 characters').refine((v) => v.length === 0 || v.length >= 10, 'Min 10 characters'),
  descriptionEn: z.string().max(500, 'Max 500 characters').refine((v) => v.length === 0 || v.length >= 10, 'Min 10 characters'),
